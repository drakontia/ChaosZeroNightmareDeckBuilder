...
function GodEffectButton({ card, effectId, cost, selected, fallback, egoLevel, hasPotential, onSelect }: { card: DeckCard; effectId: string; cost: number; selected: boolean; fallback: string; egoLevel: number; hasPotential: boolean; onSelect: () => void; }) {
  ...
  const baseInfo = getCardInfo(localizedCard, egoLevel, hasPotential, undefined, {
    ...
    translateGodEffect: (godEffectId, defaultValue) => t(`godEffects.${godEffectId}`, { defaultValue }),
    translateHiddenEffect: (hiddenEffectId, defaultValue) => t(`hiddenEffects.${hiddenEffectId}`, { defaultValue }),
  });
  ...
  const nameId = hasPersonaEngravings ? undefined : variationName ? `cards.${card.id}.name.${card.selectedHiramekiLevel}` : `cards.${card.id}.name`;
  return (
    <button className={cn("rounded-md", selected ? "ring-2 ring-primary" : "")} onClick={onSelect} title={t(`godEffects.${effectId}`, { defaultValue: effectId })}>
      <CardFrame imgUrl={baseInfo.imgUrl ?? card.imgUrl} alt={baseInfo.name} cost={cost} name={hasPersonaEngravings ? baseInfo.name : undefined} nameId={nameId} nameFallback={variationName ?? baseInfo.name} category={t(`category.${baseInfo.category ?? card.category}`)} categoryId={baseInfo.category ?? card.category} description={hasPersonaEngravings ? baseInfo.description : undefined} descriptionId={hasPersonaEngravings ? undefined : `cards.${card.id}.descriptions.${card.selectedHiramekiLevel}`} descriptionFallback={card.hiramekiVariations[card.selectedHiramekiLevel]?.description} godEffectId={effectId} godEffectFallback={fallback} statuses={baseInfo.statuses?.map((status) => t(`status.${status}`))} className="border" variant="default" />
    </button>
  );
}
  ...
  return (
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <DialogContent className={dialogContentClass}>
        <DialogHeader>
          <div className="flex items-center gap-2 pr-14"><DialogTitle className="text-left">{t("card.godSelect")}</DialogTitle><span aria-hidden>≫</span><DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline" size="default" aria-label={t("card.godSelect")}>{t(`god.${selectedGod ?? GodType.KILKEN}`)}</Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuLabel>{t("card.godSelect")}</DropdownMenuLabel>{GOD_TYPES.map((god) => <DropdownMenuItem key={god} onClick={() => setSelectedGod(god)}>{t(`god.${god}`)}</DropdownMenuItem>)}</DropdownMenuContent></DropdownMenu></div>
          <DialogCloseButton onClick={() => { props.onSetGodHirameki(props.card.deckId, null); props.onSetGodHiramekiEffect(props.card.deckId, null); props.onOpenChange(false); }} />
        </DialogHeader>
