...
test.describe("Hirameki Controls (non-character cards)", () => {
  ...
  test("shared/monster/forbidden cards show Hirameki and God controls", async ({ page }) => {
    ...
    for (const name of ["加虐性", "恥ずかしがり屋の庭師", "禁じられたアルゴリズム"]) {
      ...
      await expect(
        deckCard.getByRole("button", { name: "神ヒラメキ選択", exact: true }),
      ).toBeVisible({ timeout: 5000 });
    }
  });

  test("apply hidden hirameki to shared card updates description (and marks active)", async ({
    page,
  }) => {
    // Add a shared card
    await openAccordion(page, "共用カード");
    ...
    if (hasAccordion) {
      ...
      const tileText =
        (await firstHiddenTile.innerText())
          .split("\n")
          .map((s) => s.trim())
          .filter(Boolean)[0] ?? "";
      ...
      const updatedHiramekiBtn = updatedDeckCard.getByRole("button", {
        name: "ヒラメキ",
        exact: true,
      });
      await expect(updatedHiramekiBtn).toHaveClass(/bg-yellow-400|bg-yellow-300/, {
        timeout: 3000,
      });
    } else {
  ...
  test("apply god hirameki to shared card marks God active", async ({ page }) => {
    ...
    if (tileCount > 0) {
      ...
      const allLines = (await firstTile.innerText())
        .split("\n")
        .map((s) => s.trim())
        .filter(Boolean);
