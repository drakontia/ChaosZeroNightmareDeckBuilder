...
test.describe("Persona Card Engraving", () => {
  ...
  test("selecting a light engraving updates card name to 光のペルソナ", async ({ page }) => {
    ...
    await expect(page.getByText("光のペルソナ", { exact: true }).first()).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("can select two engravings and name becomes 光輝のペルソナ", async ({ page }) => {
    ...
    await expect(dialog).not.toBeVisible({ timeout: 5000 });
    await expect(page.getByText("光輝のペルソナ", { exact: true }).first()).toBeVisible({
      timeout: 5000,
    });
  });

  test("clicking the same engraving cycles to two slots and updates to 光輝のペルソナ", async ({
    page,
  }) => {
    await addPersonaCard(page);
