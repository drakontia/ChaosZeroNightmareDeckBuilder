...
describe("card helpers", () => {
  ...
  it("getAddableCards respects allowedJobs filtering", () => {
    const allAllowed = CARDS.find(
      (card) => card.type !== CardType.CHARACTER && card.allowedJobs === "all",
    );
    const restricted = CARDS.find(
      (card) =>
  ...
  it("returns organized image paths for representative card groups", () => {
    expect(getCardById("fei_starting_1")?.imgUrl).toBe(
      "/images/cards/character/fei_starting_1.png",
    );
    ...
    expect(getCardById("forbidden_card_1")?.imgUrl).toBe(
      "/images/cards/season1/forbidden_card_1.png",
    );
    expect(getCardById("spore_harvester")?.imgUrl).toBe(
      "/images/cards/season2/spore_harvester.png",
    );
