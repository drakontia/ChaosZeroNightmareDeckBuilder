...
import {
  getPersonaCardPresentation,
  getPersonaNameVariant,
  normalizePersonaCardEngravings,
} from "@/lib/persona";
...
describe("getPersonaCardPresentation", () => {
  it("applies a single light engraving to persona cards", () => {
    ...
    expect(result.description).toContain(
      "迅速付与、コスト1増加。行動カウントが1の対象がいる場合、このカードのコスト2減少",
    );
  });
...
describe("normalizePersonaCardEngravings", () => {
  ...
  it("不明なengraving IDは除外される", () => {
    const result = normalizePersonaCardEngravings([{ id: "unknown_id_xyz", alignment: "light" }]);
    expect(result).toEqual([]);
  });
