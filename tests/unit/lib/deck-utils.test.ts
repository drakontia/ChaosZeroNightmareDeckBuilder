...
describe("getCardInfo", () => {
  ...
  it("should derive persona presentation from persona engravings", () => {
    const personaCard: DeckCard = {
      ...
      hiramekiVariations: [
        { level: 0, cost: 1, description: "ダメージ250%", statuses: [CardStatus.UNIQUE] },
      ],
    };

    const info = getCardInfo(personaCard);
    expect(info.name).toBe("境界のペルソナ");
    expect(info.imgUrl).toBe("/images/cards/season3/persona_of_boundary.png");
  ...
  it("should preserve base persona presentation without persona engravings", () => {
    const personaCard: DeckCard = {
      ...
      personaEngravings: [],
      hiramekiVariations: [
        { level: 0, cost: 1, description: "治癒250%", statuses: [CardStatus.UNIQUE] },
      ],
    };

    const info = getCardInfo(personaCard);
    expect(info.name).toBe("ペルソナ");
    expect(info.imgUrl).toBe("/images/cards/season3/persona.png");
  ...
  it("should apply persona localization callbacks", () => {
    const personaCard: DeckCard = {
      ...
      hiramekiVariations: [
        { level: 0, cost: 1, description: "Discard up to 2", statuses: [CardStatus.UNIQUE] },
      ],
    };
